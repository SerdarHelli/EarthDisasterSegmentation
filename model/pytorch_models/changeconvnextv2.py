...
class ConvNextv2ChangeModule(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.config: ConvNextV2Config = ConvNextV2Config.from_pretrained("facebook/convnextv2-base-22k-384")
        self.ConvNext: ConvNextV2Model = ConvNextV2Model.from_pretrained("facebook/convnextv2-base-22k-384")

        hidden_size=[1024,512,256,128,128]
        mlps=[]
        for i in range(5):
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(len(features1)):
            ...
            height = width = int(features1[i].shape[-1])
