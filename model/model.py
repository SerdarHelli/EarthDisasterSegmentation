...
class USegFormerSeperated(tf.keras.Model):
    ...
    def test_step(self, inputs):
        ...
        loss_1=self.loss1_full_compute(multilabel_map,y_multilabel_resized,weights=self.class_weights)*self.loss_weights[0]
        loss_2=self.loss_2(multilabel_map,y_multilabel_resized,weights=self.class_weights)*self.loss_weights[1]
...
class UNetDouble(tf.keras.Model):
    def __init__(self, config,checkpoint_path,unet_config,unet_checkpoint_path,
                 special_checkpoint=None,
                 ):
        super(USegFormerSeperated,self).__init__()
        ...
        self.ema_momentum=config.ema_momentum
        self.gradient_clip_value=config.gradient_clip_value
        self.network=self.load_unetmodel(unet_config,unet_checkpoint_path)
        #self.segformer_layer = TFSegformerForSemanticSegmentation(config)
       # self.network=self.build_usegformer()
        self.threshold_metric=config.threshold_metric
        self.loss_weights=config.loss_weights
    ...
    def load_unetmodel(self,unet_config,unet_checkpoint_path):
        ...
        input_post_image = tf.keras.Input(shape=self.shape_input)
        input_pre_image = tf.keras.Input(shape=self.shape_input)

        unet_layer1=instantiate_from_config(unet_config.unet)
        unet_layer2=instantiate_from_config(unet_config.unet)
        unet_layer1.set_weights(unet_layer_trained.get_weights())
        unet_layer2.set_weights(unet_layer_trained.get_weights())

        
        local_map1,hiddenspre=unet_layer1(input_post_image)
        local_map2,hiddenspost=unet_layer2(input_pre_image)
        shapes=[1,2,3,4]
        hiddens=[]
        y= tf.keras.layers.Concatenate()([local_map1,local_map2])     
        for idx,(hidden_pre, hidden_post) in enumerate(zip(hiddenspre, hiddenspost)):
        ...
        model = tf.keras.Model(inputs=[input_post_image,input_pre_image], outputs=[output])
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape() as tape:
            ...
            y_multilabel_resized = self.network([x_post,x_pre], training=True)
            #upsample_resolution = tf.shape(multilabel_map)
